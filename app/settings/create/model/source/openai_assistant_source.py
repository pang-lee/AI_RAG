# '''
# Author: pang-lee
# Date: 2024-07-26 14:58:03
# LastEditTime: 2024-07-26 14:58:03
# LastEditors: LAPTOP-22MC5HRI
# Description: The openai assistant source code modify
# FilePath: \openai\application\settings\create\model\openai_assistant_source.py
# '''
...
class CustomAssistant(OpenAIAssistantRunnable):
    ...
    def modify_assistant(
        ...
        **kwargs: Any,
        ...
        update_params = {
            'name': name,
            'instructions': instructions,
            'tools': [_get_assistants_tool(tool) for tool in tools] if tools else [],
            'model': model,
            'tool_resources': tool_resources,
            **(custom_attribute or {}),
        }
    ...
    @classmethod
    def del_thread(self, thread, client: Optional[Union[openai.OpenAI, openai.AzureOpenAI]] = None):
        try:
            client = client or _get_openai_client()
            response = client.beta.threads.delete(thread)
        except Exception as e:
            raise RuntimeError(f"Thread: {thread} fail to delete {e}")
            return
    ...
    def _create_run(self, input: dict) -> Any:
        ...
        if params:        
            # 获取 run_metadata 的内容
            run_metadata = params.pop("run_metadata", {})

            # 将 run_metadata 的内容解构到 params 中
